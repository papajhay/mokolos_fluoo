...
use App\Repository\ProviderOnLineRepository;
use Doctrine\Common\Collections\ArrayCollection;
use Doctrine\Common\Collections\Collection;
use Doctrine\DBAL\Types\Types;
use Doctrine\ORM\Mapping as ORM;

#[ORM\Entity(repositoryClass: ProviderOnLineRepository::class)]
// old: FournisseurOnline
class ProviderOnLine
{
    /**
     * séparateur utilisé pour la selection de ce fournisseur
     */
    const SEPARATEUR_SELECTION = '|';

    /**
     * Nom de l'option des délai chez le fournisseur
     */
    const OPTION_ONLINE_DELAI = 'Délai de production';

    /**
     * Nom de l'option des quantité chez le fournisseur
     */
    const OPTION_ONLINE_QUANTITE = 'Tirage';

    /**
     * Nom de l'option des quantité (alternatif) chez le fournisseur
     */
    const OPTION_ONLINE_QUANTITE2 = 'Tirage/Nombre de blocs';

    /**
     * Nom de l'option des quantité (pour les textiles) chez le fournisseur
     */
    const OPTION_ONLINE_QUANTITE3 = 'Quantité';

    /**
     * code HTML du fournisseur à chaque début délément de configuration du produit
     */
    const PRODUCT_CONFIG_START_TAG = '<div class="productConfigItem opidProductConfigItem';

    /**
     * code HTML du fournisseur à chaque début délément de configuration du produit
     */
    const PRODUCT_CONFIG_ITEM_START_TAG = 'opidProductConfigItemHeadTitleText ';
...
//    id de l'option value du délai séléctionné
    #[ORM\Column]
    private ?int $idOptionValueDelaySelected = null;

//   tableau des option value de délai
    #[ORM\Column(type: Types::JSON)]
    private array $aIdOptionValueDelay = [];

//    tableau de la selection pour online
    #[ORM\Column(type: Types::JSON)]
    private array $tabSelectedOnLine = [];
...
//   old: $_aProductSelection
    #[ORM\OneToMany(mappedBy: 'providerOnLine', targetEntity: TOnlineProductRule::class)]
    private Collection $productRules;
    ...
    {
        $this->productRules = new ArrayCollection();
    }
    ...
    public function getProdIndex(): ?string
    {
        return $this->prodIndex;
    }

    public function setProdIndex(?string $prodIndex): static
    {
    ...
    public function getIdOptionValueDelaySelected(): ?int
    {
        return $this->idOptionValueDelaySelected;
    }

    public function setIdOptionValueDelaySelected(int $idOptionValueDelaySelected): static
    {
        $this->idOptionValueDelaySelected = $idOptionValueDelaySelected;

        return $this;
    }

    public function getAIdOptionValueDelay(): array
    {
        return $this->aIdOptionValueDelay;
    }

    public function setAIdOptionValueDelay(array $aIdOptionValueDelay): static
    {
        $this->aIdOptionValueDelay = $aIdOptionValueDelay;

        return $this;
    }

    public function getTabSelectedOnLine(): array
    {
        return $this->tabSelectedOnLine;
    }

    public function setTabSelectedOnLine(array $tabSelectedOnLine): static
    {
        $this->tabSelectedOnLine = $tabSelectedOnLine;

        return $this;
    }

    public function getTabSelected(): array
    {
        return $this->tabSelected;
    }

    public function setTabSelected(array $tabSelected): static
    {
    ...
    /**
     * @return Collection<int, TOnlineProductRule>
     */
    public function getProductRules(): Collection
    {
        return $this->productRules;
    }

    public function addProductRule(TOnlineProductRule $productRule): static
    {
        if (!$this->productRules->contains($productRule)) {
            $this->productRules->add($productRule);
            $productRule->setProviderOnLine($this);
        }

        return $this;
    }

    public function removeProductRule(TOnlineProductRule $productRule): static
    {
        if ($this->productRules->removeElement($productRule)) {
            // set the owning side to null (unless already changed)
            if ($productRule->getProviderOnLine() === $this) {
                $productRule->setProviderOnLine(null);
            }
        }
    ...
    public function setDataPost(array $dataPost): void
    {
        $this->dataPost = $dataPost;
    }

    public function getProductSelected(): array
    {
        return $this->productSelected;
    }

    public function setProductSelected(array $productSelected): void
    {
        $this->productSelected = $productSelected;
    }

    public function getTabSelectedTxt(): array
    {
        return $this->tabSelectedTxt;
    }

    public function setTabSelectedTxt(array $tabSelectedTxt): void
    {
        $this->tabSelectedTxt = $tabSelectedTxt;
    }

    public function getCurl(): ?object
    {
        return $this->curl;
    }

    public function setCurl(object $curl): static
    {
        $this->curl = $curl;
     ...
     * @return string
     */
    private function _pcreSubProduct()
    {
     ...
     */
    private function _masqueLigneTableau()
    {
        // initialisation du masque
        $masque	 = '#';
     ...
     * @return string le masque
     */
    private function _masqueMultiTableau()
    {
        // initialisation du masque
        $masque	 = '#';
     ...
     * @return string le masque
     */
    private function _pcreOptionTitle()
    {
        $masque	 = '#^';
     ...
     * @return string le masque
     */
    private function masqueOptionMenuDeroulant()
    {
     ...
     * @return string le masque
     */
    private function masqueOptionMenuDeroulantShort()
    {
     ...
     * @return string le masque
     */
    private function _pcreSelectOptionUnique()
    {
     ...
     * @return string le masque
     */
    private function masqueOptionMenuDeroulantPrix()
    {
        $masque = '#&euro;\s*([\d,\.]+)[^\d,\.]#';
     ...
     * @return string le masque
     */
    private function _pcreOptionTypeText()
    {
     ...
     * @return string
     */
    private function _masqueFormProduct()
    {
     ...
     * @return string le masque
     */
    private function _masqueInputHidden()
    {
     ...
     * @return string
     */
    private function masqueSelected()
    {
        return '#checked#';
    }
     ...
     * @return string
     */
    private function masqueProduitIndisponible()
    {
     ...
     * @return string
     */
    private function masqueSupplementJourOuvre()
    {
     ...
     * @return string
     */
    private function _masqueDelaiHorsMenuDeroulant()
    {
        $masque	 = '#';
     ...
     * @return string
     */
    private function _masqueDelaiMax()
    {
        return '#^(?:.*\D|)(\d+)\s*j#';
    }
     ...
     * @return string
     */
    private function _pcreProdIndex()
    {
        $pcre	 = '#';
        $pcre	 .= '\n';
        $pcre	 .= 'prodIndex\s*:\s*\'([^\']+)\'';
     ...
     * @return string
     */
    private function _pcreMailNewOrderSubject()
    {
     ...
     * @return string
     */
    private function _pcreMailNewOrderBody()
    {
     ...
     * @return string
     */
    private function _pcreMailErrorFileSubject()
    {
     ...
     * @return string
     */
    private function _pcreMailProductionSubject()
    {
     ...
     * @return string
     */
    private function _pcreMailDispatchedSubject()
    {
        $masque	 = '#';
     ...
     * @return string
     */
    private function _pcreMailOrderCanceledSubject()
    {
     ...
     * @return string
     */
    private function _pcreMailOrderConfirmationSubject()
    {
     ...
     * @return string
     */
    private function _pcreMailDispatchedBody()
    {
        $masque	 = '#';
     ...
     * @return string
     */
    private function _pcreMailResponseToARequestSubject()
    {
     ...
     * @return string
     */
    private function _pcreMailNonCompliantStructuresSubject()
    {
     ...
     * @param string $idHost id du site en cours
     * @param TProduit $product produiten cours
     * @return array un tableau avec les options et les options values
     */
    private function _createUpdateAllOptionAndOptionsValue($sourceCode, $idHost, $product)
    {
        $return		 = array();
        $keyOption	 = 0;

        // on découpe tous les élément de configuration. On supprime le premier élément qui ne correspond à rien d'interessant.
        $allConfig = explode(FournisseurOnline::PRODUCT_CONFIG_START_TAG, $sourceCode);
        unset($allConfig[0]);
        ...
        foreach($allConfig as $config)
        {
            ...
            $allItemConfig = explode(FournisseurOnline::PRODUCT_CONFIG_ITEM_START_TAG, $config);
            ...
            foreach($allItemConfig as $itemConfig)
            {
                ...
                $opionData = $this->_createUpdatOneOptionAndOptionsValue($itemConfig, $idHost, $product, $keyOption);

                // si tout c'est bien passé
                if($opionData != false)
                {
                    // on ajoute à notre tableau de retour
                    $return = array_merge($return, $opionData);
                }
     ...
     * @param string $sourceCode code source de la page fournisseur à analyser
     * @param string $idHost id du site en cours
     * @param TProduit $product produit en cours
     * @param int $keyOption id de l'option pour gérer l'ordre
     * @return array un tableau avec les options et les options values
     */
    private function _createUpdatOneOptionAndOptionsValue($sourceCode, $idHost, $product, $keyOption)
    {
        $matchOptionName		 = array();
        $matchOptionValue		 = array();
        $matchOptionValuePrice	 = array();
        $keyOptionValue			 = 0;
        ...
        if(!preg_match_all($this->masqueOptionMenuDeroulant(), $sourceCode, $matchOptionValue, PREG_SET_ORDER) && !preg_match_all($this->masqueOptionMenuDeroulantShort(), $sourceCode, $matchOptionValue, PREG_SET_ORDER) && !preg_match_all($this->_pcreSelectOptionUnique(), $sourceCode, $matchOptionValue, PREG_SET_ORDER))
        {
            ...
            return $this->_createUpdatOptionText($sourceCode, $idHost, $product, $keyOption);
        }
        ...
        {
            // option standard
            $specialOption = TOption::SPECIAL_OPTION_STANDARD;
        ...
        $return['option'] = TOption::createIfNotExist($return['name'], $this->getIdFour(), $return['name'], $order, 0, TOption::TYPE_OPTION_SELECT, $specialOption);
        TAProduitOption::createIfNotExist($product->getIdProduit(), $return['option'], $idHost);
        ...
        foreach($matchOptionValue AS $OptionMenuDeroulant)
        {
            ...
            $optionValue = TOptionValue::createIfNotExist($OptionMenuDeroulant[2], $this->getIdFour(), $return['option'], $libelleOptionValue);
            TAProduitOptionValue::createIfNotExist($product->getIdProduit(), $optionValue, $idHost, $ordre);
     ...
     * @return array un tableau avec les options et les options values
     */
    private function _createUpdatOptionText($sourceCode, $idHost, $product, $keyOption)
    {
        $matchOptionText = array();
        $return			 = array();
        ...
        foreach($matchOptionText as $dataOptionText)
        {
            ...
            $option			 = array('id' => $dataOptionText[1]);
            ...
            $option['option'] = TOption::createIfNotExist($option['id'], $this->getIdFour(), $option['name'], 40 + $keyOption, 0, TOption::TYPE_OPTION_TEXT);
            TAProduitOption::createIfNotExist($product->getIdProduit(), $option['option'], $idHost, $option['value']);
     ...
     * @return type
     */
    private function _selectionDataFromOneOptionAndOptionValue($dataOneOptionAndOptionValue)
    {
        $return = array('selection' => array(), 'selectionText' => array());
     ...
     */
    public function calculPrixAchat($prixOrigine)
    {
     ...
     */
    public function extractPrix($product, $data, $idHost)
    {
        $this->_dataPost = $data;
        ...
        $tabSelectionUtilisateur = $this->generateOrdenedSelectionByPostData($data);

        return $this->extractDelaiOuPrix($product, $tabSelectionUtilisateur, $idHost);
    }


    /**
     * Methode commune a extractDelaiPrix et extractPrix
     * @param TProduit $product				Produit
     * @param array $tabSelectionUtilisateur	Tableau de la selction
     * @param string $idHost					Id du site
     * @return type
     */
    public function extractDelaiOuPrix($product, $tabSelectionUtilisateur, $idHost)
    {
        ...
        $tabOptionValueDerniereCategorie = $this->recuperationCategorie($product, $idOptionValueOnline, $idHost, $tabDependance, 2);

        // pour chaque élément de la séléction
        foreach($tabSelectionUtilisateur AS $idOptionValue)
        {
            ...
            if(empty($tabOptionValueDerniereCategorie))
            {
                ...
                return $this->recuperationProduit($product, $idOptionValueOnline, $idHost, $tabDependance, $tabSelectionUtilisateur);
            }
            ...
            if($OptionValueOk->getOption()->getOptOrdre() < 2000)
            {
                ...
                $tabOptionValueDerniereCategorie = $this->recuperationCategorie($product, $idOptionValueOnline, $idHost, $tabDependance, 3);
            }
        }
     ...
     */
    public function checkOptionValueValide($optionValue, $tabDependance, $tabIdOptionValueValide)
    {
     ...
     * @param TProduit $product				Objet produit
     ...
     * @param int $idOptionOnline			"Level" online des liens à récupéré correspondant à une option chez nous. non utilisé dans le cas des tableaux
     * @return array un tableau des id option value de cette catégorie
     */
    private function recuperationCategorie($product, $idOptionValueOnline, $idHost, &$tabDependance, $idOptionOnline = null)
    {
        // initialisation d'un tableau qui va contenur toutes les id option value de cette catégorie
        $tabIdOptionValue	 = array();
        $resultat			 = array();
        $matchTitle			 = array();
        ...
        if(preg_match_all($this->_masqueMultiTableau(), $page, $resultat, PREG_SET_ORDER) > 0)
        {
            ...
            foreach($resultat AS $resultatTableau)
            {
                ...
                $this->analyseTableauOnline($resultatTableau[2], $resultatTableau[1], $product, $idHost, $tabDependance, $tabIdOptionValue);
            }
        }
        ...
        elseif(preg_match_all($this->_masqueLigneTableau(true), $page, $resultat) > 0 && preg_match($this->_pcreH1(), $page, $matchTitle))
        {
            ...
            // récupération des dépendance à partir de l'analyse du tableau online
            $this->analyseTableauOnline($htmlTableOnline, $matchTitle[1], $product, $idHost, $tabDependance, $tabIdOptionValue);
        }
        ...
        elseif(preg_match_all($this->_pcreSubProduct(), $page, $resultat, PREG_SET_ORDER) > 0)
        {
            ...
            TAProduitOption::createIfNotExist($product->getIdProduit(), $option, $idHost);
            ...
            foreach($resultat as $cat)
            {
                ...
                TAProduitOptionValue::createIfNotExist($product->getIdProduit(), $optionValue, $idHost);
     ...
     */
    private function recuperationProduit($product, $idOptionValueOnline, $idHost, &$tabDependance, $tabSelectionUtilisateur)
    {
        $matchProdIndex		 = array();
        $matchForm			 = array();
        $matchInputHidden	 = array();
        $matchDelay			 = array();
        ...
        $dataMenuDeroulant = $this->_createUpdateAllOptionAndOptionsValue($page, $idHost, $product);
        ...
        return $this->_recuperationProduitToutesQuantites($matchInputHidden, $product, $idHost, $tabDependance, $tabOptionValueUtilisateur, $delaiHorsMenuDeroulant);
    }
     ...
     */
    private function traitementMenuDeroulant($dataOptionAndOptionValue, $tabOptionValueUtilisateur)
    {
     ...
     * @param TProduit $product le produit
     * @param string $idHost id du site
     * @param array $tabDependance un tableau des id option value des dépendance. utilisé uniquement pour les quantités
     * @param null|int $delaiHorsMenuDeroulant nombre de jour de fabrication quand on a un nombre de jour en dehors du menu déroulant
     * @return string les données du post pour online
     */
    private function _recuperationProduitToutesQuantites($dataInputHidden, $product, $idHost, $tabDependance, $tabOptionValueUtilisateur, $delaiHorsMenuDeroulant)
    {
        ...
        $dataOptionAndOptionValueWithoutSpecialFormat = $this->_createUpdateAllOptionAndOptionsValue($pageDetailProduct['productConfig'], $idHost, $product);
        ...
        $dataOptionAndOptionValue = $this->_manageSpecialFormat($dataOptionAndOptionValueWithoutSpecialFormat, $pageDetailProduct['specialFormat'], $idHost, $product);
     ...
     */
    private function _verificationMenuDeroulantOnline($dataMenuDeroulant, $secondCheck = false)
    {
        // pour chaque menu déroulant
        foreach($dataMenuDeroulant AS $ligneMenuDeroulant)
        {
     ...
     */
    private function _manageDependencyAndPrice($aDataOptionValue, &$tabDependance, &$return, $aProductRules = array())
    {
        // si on a une option de type text
     ...
     */
    private function _manageSpecialFormat($dataOptionAndOptionValueWithoutSpecialFormat, $sourceCode, $idHost, $product)
    {
        ...
        $specialFormatOption = $this->_createUpdatOptionText($sourceCode, $idHost, $product, -1);
     ...
     */
    private function gestionPrixMenuDeroulant($OptionMenuDeroulant, &$return)
    {
     ...
     */
    private function _supplementJourOuvre($pageProduitDetail)
    {
        $resultat = array();
     ...
     * @param array $dataInputHidden tableau des données input hidden provenant de masqueInputHidden
     * @param string $idOption id de l'option online qui correspond au tableau à récupéré
     * @param array $idOptionValueOnline
     * @return string les données du post pour online
     */
    private function curlAjaxProductDetail($dataInputHidden, $idOption = null, $idOptionValueOnline = null)
    {
        $return = array();

        // paramétrage du post de la requête
        $this->getCurl()->setOptPost(true, $this->generatePostDataOnline($dataInputHidden, $idOption, $idOptionValueOnline));
     ...
     */
    private function analyseTableauOnline($codeSourceTableau, $nomOption, $product, $idHost, &$tabDependance, &$tabIdOptionValue)
    {
     ...
     * @return string les données du post pour online
     */
    private function generatePostDataOnline($dataInputHidden, $idOptionExemplaires = null, $idOptionValueExemplaires = null)
    {
     ...
     */
    private function _productRuleApplicableByOption($dataOptionAndOptionValue)
    {
        $return				 = array();
        $aOptionValueOnline	 = array();
     ...
     */
    private function _manageMailNewOrder(AchattodbEmail $achattodbEmail, $supplierOrderId)
    {
        $matches = array();
     ...
     * @return boolean TRUE si on a réussi à récupérer les infos et FALSE si on a un probléme
     */
    private function _manageMailFileError(AchattodbEmail $achattodbEmail, $supplierOrderId, $jobId)
    {
     ...
     */
    protected function _manageMailProduction(AchattodbEmail $achattodbEmail, $supplierOrderId, $jobId)
    {
        // on met à jour la commande fournisseur
     ...
     * @return boolean true si on a réussi à récupérer les infos et false si on a un probléme
     */
    private function _manageMailDispatched(AchattodbEmail $achattodbEmail, $supplierOrderId, $jobId, $trackingUrl, $trackingNumber)
    {
     ...
     */
    public function supplierOrderAutoLaunch(TSupplierOrder $supplierOrder, TLockProcess $lockProcess, $autoLaunch = true)
    {
        unset($supplierOrder);
        unset($lockProcess);
        unset($autoLaunch);
     ...
     * @return boolean TRUE si on a réussi à récupérer les infos et false si on a un probléme
     */
    private function _manageMailOrderCanceled(AchattodbEmail $achattodbEmail, $supplierOrderId, $jobId)
    {
        // on met à jour la commande fournisseur
