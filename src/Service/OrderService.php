...
class OrderService
{
    ...
    public function updateStatus(Order $order, int $status, string $comments = 'Mise &agrave jour du statut', int $modeEnvoiDuMail = OrdersStatusHistory::TYPE_ENVOI_MAIL_PAS_D_ENVOI, string $sujetMail = '', string $mailEnvoye = '', string $login = '', string $numCdeFournisseur = '', string $idDossier = '', int $typeFichier = null, int $importantComment = 0, array $pj = []): void
    {
        ...
        if (OrdersStatusHistory::TYPE_ENVOI_MAIL_MAIL_DU_STATUT === $modeEnvoiDuMail) {
            ...
            $aReplaceVariable	 = Template::replaceVariableMultiple([$futurStatut->getOrdersStatusSujet(), $futurStatut->getOrdersStatusMail()], ['order' => $this]);
        ...
        if (OrdersStatus::STATUS_CONTORLE_FICHIER_48_72_FR === $status || OrdersStatus::STATUS_CONTORLE_FICHIER_FR === $status || OrdersStatus::STATUS_CONTORLE_FICHIER_M === $status) {
        ...
        if (OrdersStatus::STATUS_FABRICATION === $status && !$this->isFullPaid($order)) {
        ...
        if (OrdersStatus::STATUS_EXPEDITION === $status && !$this->isFullPaid($order)) {
            // on change le statut pour 7.1 V.A EN FABRICATION
        ...
        if ((OrdersStatus::STATUS_LIVRE === $status || OrdersStatus::STATUS_RECLA_TRAITE === $status || OrdersStatus::STATUS_LIVRAISON_ERREUR === $status) && !$this->isFullPaid()) {
            // on change le statut pour 7.1 V.A EN FABRICATION
        ...
        if (OrdersStatus::STATUS_SUPPRESSION === $status && count($this->getReglementClient()) > 0) {
        ...
        if (OrdersStatus::STATUS_ANNULATION === $status && count($this->getReglementClient()) < 1) {
        ...
        if (OrdersStatus::STATUS_DEPART_FAB_A === $status) {
        ...
        if ((OrdersStatus::STATUS_DEPART_FAB_AUTO === $status || OrdersStatus::STATUS_DEPART_FAB_A === $status) && 1 === count($this->_aOrderSupplierOrderWithActiveSupplier($order))) {
        ...
        if (1 !== $this->getStatusFact() && (OrdersStatus::STATUS_FABRICATION === $status || OrdersStatus::STATUS_DEPART_FAB_AUTO === $status || OrdersStatus::STATUS_FABRICATION_VA === $status)) {
         ...
         */
        if (OrdersStatus::STATUS_LIVRE === $status) {
